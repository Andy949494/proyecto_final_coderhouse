...
const uploadDocuments = async (req, res) => {
    try {
        ...
        if (req.files.length > 0) {
            let documentsLinks = req.files.map(file => `https://proyectofinalcoderhouse-production-1caa.up.railway.app/documents/${file.filename}`); //la función flecha transforma cada elemento del arreglo req.files en una URL completa del documento y se almacena en el arreglo documents.
