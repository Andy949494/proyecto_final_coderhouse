...
router.post('/', uploader.array('thumbnails'), (req, res) => {
    try{
        ...
        if (req.files.length > 0) {
            let fileNames = req.files.map(file => `https://proyectofinalcoderhouse-production-1caa.up.railway.app/images/${file.filename}`); //la función flecha transforma cada elemento del arreglo req.files en una URL completa de la imagen y se almacena en el arreglo fileNames.
